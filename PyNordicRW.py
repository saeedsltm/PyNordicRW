...
"""
A simple python script for reading/writting SEISAN-NORDIC format
file/dictionary.

Written By, S.SLTM, IIEES, Teh ,2017.

Notes:

1- In the current version, It just read line types 1,4,E,F.
   
2- All variable names are consistance with SEISAN NORDIC format.
   See SEISAN V10.0 manuall, appendix A, for more information.
...
def Read_Nordic(inp='select.out', verbose=False):
    ...
    if verbose: print '+++ Read %d events successfully.'%(len(nor_dic))

    return nor_dic


def Write_Nordic(inp_dic=None, output='nordic.out'):

    output = open(output,'w')

    for evt in inp_dic:

        header = inp_dic[evt]['HEADER']
        phase  = inp_dic[evt]['PHASE']
